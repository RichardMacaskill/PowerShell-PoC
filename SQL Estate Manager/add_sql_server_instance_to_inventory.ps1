    ...
    Param(
      ...
      function discoverHosts() {
          $output = ''
          if($mode -eq "local") {
              $output = Find-DbaInstance -ComputerName $env:computername | Select-Object ComputerName,InstanceName
          } else {
              $output = Find-DbaInstance -DiscoveryType Domain | Select-Object ComputerName,InstanceName
          }
